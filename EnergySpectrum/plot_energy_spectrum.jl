...
energy_amplitudes(vecs, ρ_tot_qn) = [real(vecs[:, i]' * ρ_tot_qn * vecs[:, i]) for i in 1:size(vecs)[1]]
energy_amplitudes_matrix(vecs, ρ_tot_qn) = vecs' * ρ_tot_qn * vecs
...
function plot_spectrum(nbr_dots_res)
    ...
    for qn_res in 0:2*nbr_dots_res
        ...
        c_i2 = energy_amplitudes(vecs, ρ_tot_qn)
...
function plot_energy_amplitudes_heatmap(nbr_dots_res, qn_res)
    hams, qd_system = get_H_Ham(nbr_dots_res)
    vals, vecs = get_energy_spectrum(qn_res, hams, qd_system)
    ρ_tot_qn, E_exp = get_state_energy(qn_res, hams, qd_system)
    c_ij = energy_amplitudes_matrix(vecs, ρ_tot_qn)

    p_heatmap = plot(title = "Probabilities and coherences of energy eigenstates", xlabel = "Index", ylabel = "Index")
